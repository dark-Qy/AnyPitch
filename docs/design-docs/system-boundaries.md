...
React/Vite 工作台使用同一套 Web 代码适配桌面和手机屏幕。入口分为教练和队员：教练拥有完整的西土城FC工作台；队员只看到日程表、状态汇总和自己的参加状态。日程月历支持点击具体日程进入详情并直接维护备注与出勤，地点通过下拉和轻量管理控件维护。战术板使用 dnd-kit 拖拽站位，按赛制过滤模板，并把我方/对手 slots JSON 写回后端。
